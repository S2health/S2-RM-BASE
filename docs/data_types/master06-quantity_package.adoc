...
=== Non-quantified Types

The `Score` type is used to represent data items whose values are fiat-assigned numerics whose magnitudes carry meaning. The values imply no direct relation with real world amounts or magnitudes. This type is used for scores as described in the requirements above. Typical uses for `Score` include representing values from the Apgar and Glasgow Coma Scales, e.g. `0｜heart rate absent｜`, `4｜withdraws from pain｜`. Although the numeric values of coded ordinal data items does not strictly measure anything, they may be summed to create final scores, which classify a patient condition or situation.

`Coded_ordinal` represents data values whose values are generally Integers, and which have symbolic or text names instead, such as `"mild"`, `"medium"`, `"severe"`. Each symbol can be assigned any Integer value, providing a basis for computable comparison.  No other mathematical operations have meaning for this type.
...
The `Measured` class provides a place to record these extra data items, and may be instantiated in any instance of a descendant of `Measurable` that represents a measurement.
...
include::{uml_export_dir}/classes/{pkg}semi_quantitative.adoc[]
