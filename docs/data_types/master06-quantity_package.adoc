...
Measurement-related meta-data are not mandatory - a specific measured quantity may be stated in the same form as a stated quantity, as is typical for very common measurements such as patient vital signs.
...
Some laboratories include a normal 'status', which indicates where the value lies with respect to the normal range. The latter might take the form of markers like "HHH" (critically high), "HH" (abnormally high), "H" (borderline high), "L", "LL", "LLL" in HL7v2 messaging, although other schemes are undoubtedly used.

TODO: check if this needs to be supported.
