...
The `base.data_types.text` package contains classes for representing textual data in the health record. There are two descendant types, `Plain_text`, for representing plain text, and `Coded_text` for text items that are coded. Both types can include a subset of markdown formatting on the text element. The package is illustrated below.
...
Plain text items are used to record clinical data of two generic categories, as follows:

* 'long' narrative text:
** narrative statements that are not recorded in a structured fashion, e.g. visual observations, image interpretations;
** patient responses;
* 'short' text items:
** values that would otherwise be coded, but for which no terminology is available;
** text equivalent of quantities or dates not deemed sufficiently precise to be represented using structured quantitative or date/time date types.

For the narrative text category, a subset of markdown, including headings, paragraphs, bolding, and lists is supported via the inherited `Text.formatting` attribute being set to `markdown`.
...
* _description_: to describe a state or event, e.g. state that an intestinal infection is _listeria monocytogenes_, or that the patient has supra-ventricular tachycardia (SVT);
...
All atomic text items are either instances of the type `Plain_text` or of `Coded_text`. Within archetyped structures. The parent type `Text` should be used wherever a coded or non-coded text item is allowed; `Coded_text` should be used wherever a text item must be coded, and `Plain_text` to force plain text. The `Text` type supports optional formatting and hyperlinking (see <<_formatting_and_hyperlinking>> below).

The class `Coded_text` captures the association of two things:

* the code of a term known in the terminology service, recorded in the `_term_` attribute of type `Terminology_term`;
* the text rubric of the term, recorded in the `_value_` attribute (inherited from `Text`).

The code for a `Terminology_term` is recorded in its `concept` attribute, of type `Terminology_code`. The attribute `Terminology_code._code_string_` can record a single term code or post-coordinated code expression which may be used as an argument to a retrieval function in the terminology service to obtain the definition of the term or expression.
...
Negation is not supported as an additional Boolean property within a Coded text as in some information models. It may be safely represented in one of the following ways:

* via a pre-coordinated term that represents an exclusion, e.g `snomed_ct::409137002|No known drug allergy|`, if available;
