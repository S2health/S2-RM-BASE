...
Formatting of the text in the `_value_` field of `Dv_text` and `Coded_text` instances may be indicated via the optional `_formatting_` attribute. How this attribute is used has changed from initial versions of the openEHR Reference Model.
...
* HTML 'blocks';
* raw HTML;
* images - these may be included in openEHR content via the `Multimedia` types.
