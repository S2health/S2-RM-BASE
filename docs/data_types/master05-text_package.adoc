...
The `base.data_types.text` package contains classes for representing all textual values in the health record, including plain text, coded terms, and narrative text. It is illustrated below.
...
The model of `Coded_text` is designed to capture the _actual_ term chosen by the user or software at runtime, i.e. preferred term or synonym (for terminologies supporting synonyms), or a post-coordination of underlying distinct terms if an expression was chosen as the term (such as an expression supported by SNOMED CT). A `Coded_text` instance is used if the final textual value chosen by the user is the exact term text (preferred or other) returned by the terminology service for the key (i.e. `_code_string_` value). If the user makes even the slightest modification during data entry, a mapping (see <<Mappings>>) to a `Dv_text` should be used instead.
...
Rather than provide explicit features for representing modifier terms within `Coded_text`, the
...
terms may be nonsensical and difficult to interpret.
...
the name/value pair "allergies: NONE". Here, "allergies" will be a `Coded_text`, and "NONE"
will be either a `Coded_text` or a `Dv_text`; the two will be associated by a containing object,
...
be used to map UMLS CUIs to existing text or terms in an EHR; in this case, a `Coded_text` is
...
In cases where legacy data has to be converted to {product_name}-compliant data, and only codes are available,
...
Formatted text is represented by `Dv_text` and `Coded_text` instances.

Formatting of the text in the `_value_` field of `Dv_text` and `Coded_text` instances may be indicated via the optional `_formatting_` attribute. How this attribute is used has changed from initial versions of the {product_name} Reference Model.
...
In a concrete instance of `Dv_text`, the following are the likely usages of the `_value_` and `_formatting_` attributes.
