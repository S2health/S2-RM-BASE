...
The `base.data_types.uri` package includes two types used for referring to information resources. The `DV_URI` type allows data values which are references to objects on the world wide web to be created. Its specialisation, `DV_EHR_URI`, enables any element in an {product_name} record to be identified in the same way as other objects on the web. The `DV_EHR_URI` type is convenient, because it is a string, like any other URI, and is therefore easily transportable and processable. Because it has its own scheme space, 'ehr', instances can be globally unique, as long as EHR identification is globally unique. `DV_EHR_URIs` are used to express all runtime paths in the EHR. The `uri` Package is illustrated below.

[.text-center]
.base.data_types.uri package
image::{uml_diagrams_uri}/BASE-data_types.uri.svg[id=base_data_types_uri, align="center"]
...
include::{uml_export_dir}/classes/{pkg}uri_ref.adoc[]

include::{uml_export_dir}/classes/{pkg}ehr_uri_ref.adoc[]

== Syntaxes

=== DV_EHR_URI Syntax

The syntax of a `DV_EHR_URI` is an {product_name} path, inside the 'ehr' URI scheme-space, and is one of the forms:
...
The syntax is described in more detail in the {openehr_overview}#_ehr_uris[{product_name} Architecture Overview]. `DV_EHR_URIs` are used as a mechanism for referencing in the EHR, ensuring readability by humans, as well as validity when extracts are transmitted elsewhere: even if the target of a path is not present, the path can be used to locate the missing item on demand.
