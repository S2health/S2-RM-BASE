...
== Overview
The `foundation_types.encapsulated` package contains classes representing data values whose internal
structure is defined outside the EHR model, such as multimedia and parsable data. It is illustrated below.

[.text-center]
.base.foundation_types.encapsulated package
image::{uml_diagrams_uri}/BASE-foundation_types.encapsulated.svg[id=foundation_types_encapsulated, align="center"]
...
These observations lead naturally to an abstract `Encapsulated` class, with two subtypes,
`Parsable`, for all content which is syntactic in nature, and `Multimedia` for everything else.
...
purposes be represented as instances of `Multimedia`, rather than `Parsable`. The vast
