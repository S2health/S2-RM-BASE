...
The primitive Time types defined here are represented by classes defined in the package `base.foundation_types.time`. These are concrete types based on the {iso_8601_official}[ISO 8601 (2019) date/time standard^] semantics, which supports partial dates, times, and complex durations, all of which are needed in the biomedical and clinical domains. All types inherit a `String` physical representation from the `Temporal` class, and the Gregorian calendar is assumed.
...
** If not even the year is known, then the date is obviously extremely approximate and it may be unsafe to represent it computationally.
...
* 'expanded' dates, which have year numbers of greater than 4 digits, and may be negative; in {product_name}, only 4-digit year numbers are assumed;
...
* partial date/times with fractional minutes or hours, e.g. `hh,hhh` or `mm,mm`; in {product_name}, only fractional seconds are supported;
* the interval syntax. Intervals of date/times are supported in {product_name}, but their syntax form is defined by ADL, and is standardised across all comparable types, not just dates and times.

Deviations from the published standard include the following.

* Durations are supposed to take the form of `PnnW` or `PnnYnnMnnDTnnHnnMnnS`, but in {product_name}, the W (week) designator can be used in combination with the other designators, since it is very common to state durations of pregnancy as some combination of weeks and days.
* Durations may take a negative sign, indicating a duration prior to an origin. This supports healthcare needs such as 'adjusted age' (e.g. '-P3M', an age of 'minus 3 months') for very premature newborns.
...
** the same deviation is used in HL7v2 timestamps, i.e. there are data in existing clinical systems matching this specification;
...
* The time `24:00:00` (or `240000`) is not allowed anywhere, whereas in ISO 8601:2019 it appears to be legal at least for pure times. This deviation is also appears to be used in HL7v2 (where midnight is defined as the time `00:00:00`), and is preferable to the documented standard, since a date/time with time of `24:00:00` is really the next day, i.e. the date part is then incorrect.
...
include::{uml_export_dir}/classes/{pkg}date.adoc[]

include::{uml_export_dir}/classes/{pkg}time.adoc[]

include::{uml_export_dir}/classes/{pkg}date_time.adoc[]

include::{uml_export_dir}/classes/{pkg}duration.adoc[]

include::{uml_export_dir}/classes/{pkg}timezone.adoc[]
