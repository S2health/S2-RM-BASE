...
This section describes a generic interval class, used ubiquitously in data within healthcare and science more generally. The definition of the `Interval<T>` class here is an intensional one, i.e. it states its members by implication from limits, rather than enumerating them. To support the common need for defining times in models that could be either a fixed point in time or a time interval, the classes `Point_interval<T>` and `Proper_interval<T>` are provided. If `Interval<X>` is defined as the type of a feature in a class in an {product_name} model, where `X` is some descendant of `Comparable`, then at runtime, either a `Point_interval` or `Proper_interval` may be attached.
